"""
pmcprg.copulas._bivariate_fit — IFM fit result, bootstrap, GoF, and diagnostics
...
class BivariateFitResult:
    ...
    def plot_diagnostics(self, plot_dir: str, prefix: str = "") -> None:
        ...
        ax = axes[0, 0]
        # Evaluate the density over the *data's* extent rather than the law's
        # default 5–95 % quantile grid: under a full scatter, that grid left a
        # tenth of the points outside the shaded rectangle, which reads as
        # "the model puts no density here".
        x0, x1 = float(self.data[:, 0].min()), float(self.data[:, 0].max())
        y0, y1 = float(self.data[:, 1].min()), float(self.data[:, 1].max())
        pad_x, pad_y = 0.03 * (x1 - x0), 0.03 * (y1 - y0)
        GXd, GYd = np.meshgrid(
            np.linspace(x0 - pad_x, x1 + pad_x, 160),
            np.linspace(y0 - pad_y, y1 + pad_y, 160),
        )
        Z = np.vectorize(lambda a, b: self.bivariate.pdf([a, b]))(GXd, GYd)
        vmax = float(np.nanpercentile(Z, 95))
        ax.contourf(
            GXd,
            GYd,
            Z,
            # Explicit levels capped at p95 (see pmcprg/copulas/_fit.py): an
            # integer spreads the levels over the raw range of a density that
            # spans decades, collapsing the panel to one flat colour.
            levels=np.linspace(0, max(vmax, 1e-10), 10),
            cmap="Blues",
            alpha=0.45,
            extend="max",
            vmin=0,
