...
def test_degenerate_step_falls_back_to_uniform():
    """A zero-mass (underflowed) step yields uniform posteriors — 1/K from
    ``smooth`` and 1/K² from ``joint_posteriors`` — so both stay valid
    distributions and the γ = Σ_j ξ invariant survives (audit N-8 / N-11)."""
    K, N = 3, 4
    alpha = np.full((N, K), 1.0 / K)
    beta  = np.ones((N, K))
    W     = np.full((N - 1, K, K), 1.0 / K)
    W[1]  = 0.0                                        # degenerate step
    beta_z        = beta.copy()
    beta_z[2]     = 0.0                                # degenerate γ row

    xi = joint_posteriors(alpha, W, beta)
    np.testing.assert_allclose(xi[1], 1.0 / K**2)
    np.testing.assert_allclose(xi.sum(axis=(1, 2)), 1.0)

    gamma = smooth(alpha, beta_z)
    np.testing.assert_allclose(gamma[2], 1.0 / K)
