...
def test_select_margin_family_recovers_strongly_skewed_gamma(rule):
    """Moderately-skewed Gamma data: **every** rule must pick Gamma.

    ``aic``/``bic`` used to be excused here as "a known conservative
    trade-off": they scored a per-observation *mean* log-density against the
    absolute penalties ``2k`` and ``k·log n``, so the penalty was ~n times too
    strong and the fewest-parameter family (Gauss) won by construction. With
    the total weighted log-likelihood and Σw as the effective sample size,
    that excuse no longer applies at n = 2000.
    """
...
def test_ice_logs_gice_at_debug(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="pmcprg.pmc.ice"):
        ice(mdl, Y, ice_cfg={
...
# ---------------------------------------------------------------------------
# Regression: _kolmogorov_distance must handle tied observations (audit N-4)
# ---------------------------------------------------------------------------

def test_kolmogorov_distance_handles_ties():
    """On tied (quantised) data the weighted ECDF must be right-continuous:
    all equal y share one F_n value. The previous argsort+cumsum gave tied
    points distinct partial sums, fabricating spurious KS deviations."""
    from scipy import stats as _ss

    y = np.array([1.0, 1.0, 1.0, 2.0, 2.0, 3.0])
    w = np.ones_like(y)
    params = {"loc": 2.0, "scale": 1.0}

    # Reference right-continuous weighted ECDF: F_n(y) = Σ_{m: y_m ≤ y} w_m / Σ w.
    w_norm   = w / w.sum()
    cdf_emp  = np.array([w_norm[y <= v].sum() for v in y])
    cdf_th   = _ss.norm.cdf(y, **params)
    expected = float(np.max(np.abs(cdf_th - cdf_emp)))

    got = _kolmogorov_distance("norm", params, y, w)
    assert got == pytest.approx(expected, abs=1e-12)

    # All three y == 1.0 share the same ECDF value → the max deviation is taken
    # against F_n = 3/6 there (not 1/6 or 2/6 as the buggy per-position cumsum
    # would yield); a sanity check that ties were collapsed.
    assert cdf_emp[0] == cdf_emp[1] == cdf_emp[2]
