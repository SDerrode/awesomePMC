...
def test_classify_image_recovers_synthetic_segmentation():
    ...
    """
    from pmcprg.pmc import PMCModel, simulate, signal_to_image, classify_image, error_rate

    mdl0 = PMCModel("pmcprg/pmc/models/hmc_in_gauss_k2.toml")
...
def test_ice_image_runs():
    ...
    from pmcprg.pmc import PMCModel, simulate, signal_to_image, ice_image
...
def test_load_save_roundtrip_through_png(tmp_path):
    ...
    from pmcprg.pmc.peano import load_grayscale, load_labels, save_segmentation
...
def test_classify_image_rejects_non_2d():
    ...
    from pmcprg.pmc import PMCModel, classify_image
    mdl = PMCModel("pmcprg/pmc/models/hmc_in_gauss_k2.toml")
...
def test_ice_image_rejects_non_2d():
    """A scalar (d=1) model rejects a multi-channel image: dimension mismatch."""
    from pmcprg.pmc import PMCModel, ice_image
    mdl = PMCModel("pmcprg/pmc/models/hmc_in_gauss_k2.toml")
    with pytest.raises(ValueError, match="Image channels"):
