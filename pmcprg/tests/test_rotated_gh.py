"""90°/270°-rotated Gumbel-Hougaard (GH) copulas (audit FR-8, second round).

Mirrors ``test_rotated_clayton.py``'s depth for the GH base family: GH already
exposed the kernel interface (used by ``SurvivalGH`` in ``survival.py``), so
``CopulaGH90``/``CopulaGH270`` are the same two-line ``RotatedCopula90``/
``RotatedCopula270`` subclasses as Clayton's, registered as
``CopulaEnum.GH90``/``GH270``.

Complements ``test_copula_limits.py`` (which adds GH90/GH270 to its own grid
and high-precision reference file, derived from the *already validated* base
GH's own decimal references reflected through
...
from pmcprg.copulas import CopulaGH, CopulaGH90, CopulaGH270, CopulaEnum
from pmcprg.exceptions import CopulaParameterError
from pmcprg.numerics import EPS

GH90 = CopulaEnum.GH90
GH270 = CopulaEnum.GH270
...
@pytest.mark.parametrize("entry,short,cls_name,klass", [
    (GH90, "GH90", "CopulaGH90", CopulaGH90),
    (GH270, "GH270", "CopulaGH270", CopulaGH270),
])
...
@pytest.mark.parametrize("entry", [GH90, GH270])
def test_tau_range_is_negative_only(entry):
    """Mirror of GH's own [0+EPS, 1.0] one-sided range (audit FR-8)."""
    lo, hi = entry.value.TAU_MIN_MAX
    assert lo == pytest.approx(-1.0)
    assert hi == pytest.approx(-EPS)


@pytest.mark.parametrize("cls", [CopulaGH90, CopulaGH270])
...
def test_gh90_cdf_matches_vinecopula_identity(tau):
    """C90(u,v) == v − C_base(1−u, v) on a 25×25 grid, to machine precision."""
    base = CopulaGH(tau_k=-tau)
    rot = CopulaGH90(tau_k=tau)
    u, v = _GRID[:, 0], _GRID[:, 1]
...
@pytest.mark.parametrize("tau", _TAUS)
def test_gh270_cdf_matches_vinecopula_identity(tau):
    """C270(u,v) == u − C_base(u, 1−v) on a 25×25 grid, to machine precision."""
    base = CopulaGH(tau_k=-tau)
    rot = CopulaGH270(tau_k=tau)
    u, v = _GRID[:, 0], _GRID[:, 1]
    lhs = rot.cdf_array(_GRID)
...
@pytest.mark.parametrize("tau", _TAUS)
def test_gh90_pdf_matches_base_at_reflected_point(tau):
    """c90(u,v) == c_base(1−u, v), the stronger, cheaper check the task asks
    for: reuse the already-validated base GH density instead of a fresh
    mpmath ground truth."""
    base = CopulaGH(tau_k=-tau)
    rot = CopulaGH90(tau_k=tau)
    u, v = _GRID[:, 0], _GRID[:, 1]
...
@pytest.mark.parametrize("tau", _TAUS)
def test_gh270_pdf_matches_base_at_reflected_point(tau):
...
@pytest.mark.parametrize("tau", _TAUS)
def test_gh90_h_matches_base_h_with_reflected_conditioning(tau):
...
@pytest.mark.parametrize("tau", _TAUS)
def test_gh270_h_matches_one_minus_base_h_with_reflected_v(tau):
...
def test_theta_matches_base_gh_at_minus_tau(cls, tau):
    """θ_rot == θ_GH(−τ) — the rotation only negates τ, θ itself is the
...
def test_sample_has_uniform_margins_and_the_right_negative_tau(tau):
    for cls in (CopulaGH90, CopulaGH270):
        cop = cls(tau_k=tau)
...
def test_90_and_270_are_different_copulas_with_swapped_tail_asymmetry():
    ...
    of the swap is the mirror image of the Clayton pilot's, because GH's
    own tail asymmetry is upper (mass at (1,1)), not lower (mass at (0,0))
    like Clayton's: reflecting (U,V)~GH (concentrated near (1,1)) through
    (1−U,V) pushes that mass to (0,1), the upper-left corner, so GH90
    concentrates *there*, not near the lower-right like Clayton90; GH270
    is the mirror image, near the lower-right (u→1, v→0). Verified directly
    below (a plain GH sample at τ=0.6 has corner mass ≈0.072 at (1,1) vs.
    ≈0.047 at (0,0), the asymmetry GH90/GH270 relocate) — this is the one
    sampling-based corner check run for the pair; the stronger, cleaner
    version of it (closer to the pilot's own 2.7× ratio at the pilot's own
    threshold) is run once more, on Joe, in ``test_rotated_joe.py``, whose
    upper-tail asymmetry is sharper than GH's at the same τ."""
    tau = -0.6
    s90 = CopulaGH90(tau_k=tau).sample(n=40000, seed=7)
    s270 = CopulaGH270(tau_k=tau).sample(n=40000, seed=7)

    def corner_masses(uv, t=0.03):
    ...
    # GH90: upper-left corner should dominate (GH's upper-tail dependence
    # reflected through the first margin only) — opposite of Clayton90.
    assert ul90 > 2.0 * lr90, f"GH90 corners: upper-left={ul90}, lower-right={lr90}"
    # GH270: lower-right corner should dominate (mirror image of 90°).
    assert lr270 > 2.0 * ul270, f"GH270 corners: upper-left={ul270}, lower-right={lr270}"
...
def test_fit_best_includes_gh_rotations():
    from pmcprg.copulas._base import CopulaVirt
    cop = CopulaGH90(tau_k=-0.6)
    uv = cop.sample(n=800, seed=3)
    results = CopulaVirt.fit_best(uv, families=[CopulaGH90, CopulaGH270], method="tau")
    assert len(results) == 2
    # The correctly-rotated family should fit strictly better (lower AIC).
    assert results[0].copula.__class__ is CopulaGH90
...
def test_gh_rotations_are_covered_by_the_reference_file():
    from test_copula_limits import (
        _TAIL_TAUS, _INDEP_TAUS, _file_table, _required_param_keys, _match_file_key,
    )
    for short in ("GH90", "GH270"):
        assert short in _TAIL_TAUS
        assert short in _INDEP_TAUS
    table = _file_table()
    need = {k for k in _required_param_keys() if k[0] in ("GH90", "GH270")}
    assert need, "no GH90/GH270 cases registered in test_copula_limits.py"
    for key in need:
