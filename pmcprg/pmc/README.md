...
| Feature                                               | Source           |
| ----------------------------------------------------- | ---------------- |
| The PMC model family, SR-PMC reversibility, state vs pair margins (§2.1 Proposition) | A16, §2 |
...
| **ICE** — unsupervised parameter estimation. *ξ-weighted variant*: the papers estimate copulas and margins on **one** posterior draw (L = 1, A16 Eqs. 22–24; A23 §3 (b)–(c)); `sem()` is the hard-draw estimator. See the `ice.py` docstring. | A16, §4.2 |
...
numerical L-BFGS-B fit for every margin candidate instead. The ICE M-step is
the ξ-weighted (conditional-expectation) variant rather than the one-draw
scheme of A16 §4.2 / A23 §3 — see above. Otherwise the methodology of both
...
| File           | Role                                                        |
| -------------- | ----------------------------------------------------------- |
| `model.py`     | TOML schema + `PMCModel` validation. Margin structure "state" (f_i, X Markov) or "pair" (f_ij, general PMC, A16 Eqs. 12–14).                                         |
...
unit-test suite (`pmcprg/tests/test_gice_margins.py`) on the bundled
