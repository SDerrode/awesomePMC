"""
logging_setup.py — Centralised logging configuration for pmcprg.pmc.
...
Log file location
-----------------
Default: ~/.awesomepmc/pmc.log  (created automatically).
Override by passing log_file=Path(...) to configure().
Pass log_file=False to disable file logging entirely.

Record format (file)
--------------------
  2026-05-04 12:34:56 DEBUG    pmcprg.pmc.ice — _neg_wll: copula eval failed …
  2026-05-04 12:34:56 WARNING  pmcprg.pmc.inference — Forward: C_1=0 at n=0 …
...
# Root logger for the whole pmcprg namespace
_PRG_LOGGER = logging.getLogger("pmcprg")
...
def configure(
    ...
    """
    Configure the ``pmcprg`` logger with a console handler and a file handler.

    Parameters
    ----------
    level      : int   Console log level (default WARNING).
    log_file   : Path  Log file path. Defaults to ``~/.awesomepmc/pmc.log``.
    ...
    if log_file is not False:
        if log_file is None:
            log_file = Path.home() / ".awesomepmc" / "pmc.log"
