...
-----
  ...
  # From Python:
  from pmcprg.pmc.gui import run_gui
  run_gui()
  run_gui("my_model.toml")

Logging
-------
The GUI sets up file logging to ~/.awesomepmc/pmc.log (DEBUG level) and
attaches a QTextEdit handler so every log record from the pmcprg.* namespace
...
def run_gui(model_path: str | None = None) -> int:
    ...
    """
    # ── Logging: file handler first (before any pmcprg import that logs) ────
