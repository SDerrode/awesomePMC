...
PMC of A16 Eqs. 12–14). ``model.margin(i)`` raises on a pair model, so every
panel of the GUI that speaks of "the margin of state i" goes through
:func:`state_law` — f_i on a state model, and on a pair model the law of y_n
given x_n = i alone, g_i = Σ_j (p_ij / p_i) f_ij.
...
*Computational Statistics & Data Analysis* 63, 81–98 ("A16").
"""
...
def margin_cdfs(mdl, Y, *, lo: float, hi: float) -> np.ndarray:
    ...
    f_ij and the right margin f_ji of A16 Eq. 12.
    """
