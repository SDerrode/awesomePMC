...
from prg.pmc.ice import _fit_multivariate_gaussian_weighted
...
def test_simulate_emits_scalar_y_for_d_eq_1():
    m = PMCModel("prg/pmc/models/hmc_in_gauss_k2.toml")
    X, Y = simulate(m, N=200, seed=0)
...
def test_classify_image_rgb_synthetic():
    ...
    m = PMCModel("prg/pmc/models/hmc_in_mvn_k2_d3.toml")
    H, W = 32, 48; N = H * W
...
def test_classify_image_rejects_dimensionality_mismatch():
    ...
    m1d = PMCModel("prg/pmc/models/hmc_in_gauss_k2.toml")
    with pytest.raises(ValueError, match="Image channels"):
...
def test_ice_image_recovers_means_from_perturbed_init():
    ...
    m_true = PMCModel("prg/pmc/models/hmc_in_mvn_k2_d3.toml")
