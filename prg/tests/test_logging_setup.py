"""
test_logging_setup.py — coverage for ``prg.pmc.logging_setup``.
...
from prg.pmc import logging_setup
...
def test_configure_explicit_path(fresh_logger, tmp_path: Path):
    ...
    logging.getLogger("prg.pmc.test").error("hello world")
...
def test_widget_handler_receives_records(fresh_logger, tmp_path: Path, qapp):
    ...
    try:
        logging.getLogger("prg.pmc.test").info("message-from-test")
        for h in fresh_logger.handlers:
            h.flush()
