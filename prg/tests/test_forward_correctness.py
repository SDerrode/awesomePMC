...
returned by ``prg.pmc.inference.forward`` matches the true value computed
...
from prg.pmc.inference import forward
...
def _brute_force_log_lik(mdl: PMCModel, Y: np.ndarray) -> float:
    ...
    Uses the SR-PMC joint factorisation:
...
def test_forward_matches_brute_force(toml_path: Path, seed: int):
    ...
    assert abs(ll_fwd - ll_ref) < _FWD_BRUTE_TOL, (
        ...
        f"prg/pmc/inference.py:precompute_weights."
    )
