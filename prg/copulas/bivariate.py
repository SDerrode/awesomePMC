...
from prg.tools.tools            import minmaxEPS, EPS, list_parameters
...
def _margin_to_dict(margin: tuple) -> dict:
    ...
    return {
        ...
        'param_names': list_parameters(dist),
    }
...
class BivariateLaw:
    ...
    def _copula_uvs(self, y_cond: float, y_obs: float,
                    which: str) -> tuple[float, float, float]:
        ...
        """
        if which == 'left':
            u_obs  = minmaxEPS(self.left_margin['dist'].cdf(y_obs,   *self.left_margin['params']))
            u_cond = minmaxEPS(self.right_margin['dist'].cdf(y_cond, *self.right_margin['params']))
            lf     = float(self.right_margin['dist'].logpdf(y_cond,  *self.right_margin['params']))
        elif which == 'right':
            u_obs  = minmaxEPS(self.right_margin['dist'].cdf(y_obs,  *self.right_margin['params']))
            u_cond = minmaxEPS(self.left_margin['dist'].cdf(y_cond,  *self.left_margin['params']))
            lf     = float(self.left_margin['dist'].logpdf(y_cond,   *self.left_margin['params']))
        else:
            raise ValueError("which must be 'left' or 'right'.")
        return u_obs, u_cond, lf

    def conditional_log_pdf(self, y_cond: float, y_obs: float,
                            which: str = 'left') -> float:
    ...
    def sample_conditional(self, y_obs: float, which: str = 'left',
                           n: int = 1) -> np.ndarray:
    ...
    def _sample_left(self) -> float:
        return float(self.left_margin['dist'].rvs(
    ...
    def _sample_right_given_left(self, y_left: float) -> float:
        ...
        while cpt < _NBITERMAX:
            cpt    += 1
            y_right = float(self.right_margin['dist'].rvs(
    ...
    def _sample_left_given_right(self, y_right: float) -> float:
        ...
        while cpt < _NBITERMAX:
            cpt   += 1
            y_left = float(self.left_margin['dist'].rvs(
                *self.left_margin['params'], random_state=self._rng))
            u_left = minmaxEPS(
                self.left_margin['dist'].cdf(y_left, *self.left_margin['params']))
            uv[0]     = u_left
    ...
    def fit(cls, data: np.ndarray,
            copula_class, left_family, right_family,
        ...
        for fam, label in ((left_family, 'left_family'),
                           (right_family, 'right_family')):
            if not all(hasattr(fam, m) for m in ('pdf', 'cdf', 'logpdf', 'rvs', 'ppf', 'fit')):
        ...
        bivariate = cls(copula_fit.copula,
        ...
        return BivariateFitResult(
            bivariate=bivariate, copula_fit=copula_fit,
            left_params=left_params, right_params=right_params,
            log_likelihood=ll, n_obs=n, data=data,
        )

    @staticmethod
    def fit_best(data: np.ndarray, left_family, right_family,
        ...
        if copula_families is None:
            ...
            copula_families = [CopulaGaussian, CopulaStudent, CopulaGH, CopulaClayton,
                               CopulaA12, CopulaA14, CopulaFGM, CopulaCubSec]

        results: list = []
        for cls in copula_families:
            try:
                results.append(BivariateLaw.fit(data, cls, left_family, right_family,
    ...
    def plot_pdf(self, plot_dir: str, prefix: str = '') -> None:
        ...
        min_ = np.nanpercentile(z, 1.)
        max_ = np.nanpercentile(z, 99.)
        vticks = np.linspace(min_, max_, num=self.ticks_nbr)
        cs = ax.contourf(self._X, self._Y, z, vticks, antialiased=True, vmin=min_, vmax=max_)
        ...
        plt.savefig(os.path.join(plot_dir, f'{prefix}2D_PdfLaw.png'),
                    bbox_inches='tight', dpi=dpi, facecolor=facecolor)
        plt.close()

    def plot_pdf_with_margins(self, plot_dir: str, title: str = '',
                              prefix: str = '') -> None:
        z = self._compute_pdf_grid()
        ...
        ax_margin_left.plot(self._x,
                            self.left_margin['dist'].pdf(self._x, *lparams),
                            'b-', lw=2, alpha=0.6)
        ax_margin_right.plot(self.right_margin['dist'].pdf(self._y, *rparams),
                             self._y, 'b-', lw=2, alpha=0.6)

        lname = ', '.join(f'{v:.2f}' for v in lparams)
        rname = ', '.join(f'{v:.2f}' for v in rparams)
        ax_margin_left.text(0.5, 0.9,
                            f"{self.left_margin['dist_name']} ({lname})",
                            transform=ax_margin_left.transAxes, va='center', ha='center')
        ax_margin_right.text(0.9, 0.5,
                             f"{self.right_margin['dist_name']} ({rname})",
                             transform=ax_margin_right.transAxes, va='center', ha='center',
                             rotation=270)
        ...
        plt.savefig(os.path.join(plot_dir, f'{prefix}2D_PdfLawWithMarginPdfs.png'),
                    bbox_inches='tight', dpi=dpi, facecolor=facecolor)
        plt.close()
...
class BivariateBootstrapCI:
    ...
    def __repr__(self) -> str:
        ...
        return (f'BivariateBootstrapCI({100*(1-self.alpha):.0f}%, '
...
class BivariateFitResult:
    ...
    def n_params(self) -> int:
        return (len(self.left_params) + len(self.right_params)
                + self.copula_fit.n_params)
    ...
    def __repr__(self) -> str:
        ...
        return (f'BivariateFitResult(copula={cop!r}, '
    ...
    def gof_test(self, B: int = 100, seed: int | None = None) -> GoFResult:
        ...
        for b in range(B):
            ...
            try:
                r_b      = BivariateLaw.fit(sample, cop_class,
    ...
    def bootstrap_ci(self, B: int = 500, alpha: float = 0.05,
                     seed: int | None = None) -> BivariateBootstrapCI:
        ...
        tau_ci = (float(np.quantile(tau_boots[valid], q_lo)),
        ...
        return BivariateBootstrapCI(tau_k=tau_ci, left_params=left_ci,
                                    right_params=right_ci, alpha=alpha,
                                    B=B, n_valid=int(valid.sum()))
    ...
    def cv_loglik(self, K: int = 5, seed: int | None = None) -> float:
        ...
        for k in range(K):
            ...
            try:
                r_k = BivariateLaw.fit(self.data[train_idx], cop_class,
                                       left_family, right_family,
                                       copula_method=method)
                for i in test_idx:
    ...
    def plot_diagnostics(self, plot_dir: str, prefix: str = '') -> None:
        ...
        ax.contourf(self.bivariate._X, self.bivariate._Y, Z,
                    levels=10, cmap='Blues', alpha=0.45,
        ...
        try:
            ...
            ax.set_xlabel('$F_n$ (empirical)'); ax.set_ylabel(r'$F_\theta$ (fitted)')
            ax.set_title('Joint PP plot', fontsize=BIGGER_SIZE)
        except NotImplementedError:
            ax.axis('off')
            ax.text(0.5, 0.5, 'Joint CDF not available', ha='center', va='center',
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, f'{prefix}BivariateDiagnostics.png'),
                    bbox_inches='tight', dpi=dpi, facecolor=facecolor)
        plt.close()
